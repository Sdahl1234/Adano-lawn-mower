...
async def validate_input(hass: core.HomeAssistant, name, email, password):
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for Adano mower integration."""
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            try:
                ...
                await validate_input(self.hass, name, email, password)
                unique_id = f"{name}"
